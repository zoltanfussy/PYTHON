...
open('proteom_only.fasta', 'w') as outfasta:
	...
	for i in cvel_plastNN_set & cvel_pt_set:
		outfile3.write("{}\t{}\n".format(i, fasta_d[i]["desc"]))
	outfile3.write("\n\n\n\nplastNN ONLY:\n")
	NNfxset = set([fasta_d[i]["desc"] for i in cvel_plastNN_set - cvel_pt_set])
	ASfxset = set([fasta_d[i]["desc"] for i in cvel_pt_set - cvel_plastNN_set])
	NNfxlist = list(NNfxset - ASfxset)
	NNfxlist.sort()
	print("plastNN: {} functions".format(len(NNfxlist)))
	outfile3.write("\n".join(NNfxlist))
	#for i in cvel_plastNN_set - cvel_pt_set:
		#outfile3.write("{}\t{}\n".format(i, fasta_d[i]["desc"]))
		#for item in fasta_d[i]["models"]:
		#	outfile3.write(">{} {}\n{}\n".format(item, fasta_d[i]["desc"], seq_d[item]))
	ASfxlist = list(ASfxset - NNfxset)
	ASfxlist.sort()
	print("ASAFind: {} functions".format(len(ASfxlist)))
	outfile3.write("\n\n\n\nASAFind ONLY:\n")
	outfile3.write("\n".join(ASfxlist))
