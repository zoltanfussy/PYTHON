...
allowed = ("fasta", "fas", "fst", "fa", "faa", "fna", "phy", "phylip", "ali")
...
for file in infilelist:
	...
	if extension in ("fasta", "fas", "fst", "fa", "faa", "fna", "ali"):
		try:
	...
	if args.aligner == "run_pasta.py":
		...
		print("Aligner switched off, trimming skipped!")
		pass
		...
		if args.treemaker in ["iqtree-omp", "iqtree"]:
			if os.path.isfile("trimfilt-{}.fasta.treefile".format(filename)):
				...
				if args.no_guide:
					...
					print("FORREST: Issuing software for guide tree inference:\n{} {} {}".format(program, guidetreecommand, logtofile))
				...
				print("FORREST: Issuing software for final tree inference:\n{} {} {}".format(program, treecommand, logtofile))
	...
	if iqtree_convergence_error("final-{}_iqtree.log".format(filename)):
		...
		os.system("{} {} -nm 4000 1>final2-{}_iqtree.log".format(program, treecommand, filename))
