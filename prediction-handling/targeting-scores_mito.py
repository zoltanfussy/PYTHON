...
if os.path.exists(prefix + ".mitofates.txt"):
	print("Found MitoFates output")
	mitofates = open(prefix + ".mitofates.txt").read().split('\n')
	...
	for item in mitofates:
		...
		if not item.startswith('Sequence ID') and len(item) != 0:
			...
			preds_d.at[name, "MitoFates"] = pred
...
if os.path.exists(prefix + ".ML2_animalLO.txt"):
	print("Found MultiLoc2-animal output")
	ML2ANIM = open(prefix + ".ML2_animalLO.txt").read().split('\n')
...
if os.path.exists(prefix + ".ML2_fungalLO.txt"):
	print("Found MultiLoc2-fungal output")
	ML2ANIM = open(prefix + ".ML2_fungalLO.txt").read().split('\n')
	possiblepredsml2 = {'cytoplasmic': "CYT", "mitochondrial": "MT", "nuclear": "NC", 
	"secretory pathway": "SEC"}
	if ML2ANIM[0].startswith("MultiLoc2"):
		ML2ANIM = ML2ANIM[5:]
...
preds_d.to_csv(path_or_buf='{}_merged-scores.tsv'.format(prefix), float_format='%0.3f', sep="\t")
