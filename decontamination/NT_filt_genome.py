...
for file in files:
			...
			if len(line.split("\t")) != 1:
				...
				if taxid == "N/A":
					...
					try:
						...
						print(f"Invalid taxid, force checking: {line}")
						taxid = force_taxid_nucl(accession)
						lineage = ncbi.get_lineage(taxid)[2:]
	...
	outfile = open("{0}_NT/{0}.{1}.NTfilt.fasta".format(dataset, transcriptome), "w")
	out_bact = open("{0}_NT/{0}_bact.fasta".format(dataset), "w")
	out_fungal = open("{0}_NT/{0}_fungal.fasta".format(dataset), "w")
	out_animal = open("{0}_NT/{0}_animal.fasta".format(dataset), "w")
	out_plant = open("{0}_NT/{0}_plant.fasta".format(dataset), "w")
	out_para = open("{0}_NT/{0}_para.fasta".format(dataset), "w")
	out_other = open("{0}_NT/{0}_other.fasta".format(dataset), "w") #assuming this organism is not in nr
	for seq in infasta:
