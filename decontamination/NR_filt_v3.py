...
def assembly_methods(blastline):
	...
	if seqname.startswith("TRINITY"):
		...
		if seqname.split("_")[-3] == "cov":
			#format >NODE_177_length_94900_cov_24.327530
			#prodigal appends a _number to seqid
			predictor = "prodigal"
	...
	print(assembler, predictor)
	return assembler, predictor


def get_scaffold(predictor, seqname):
	if predictor == "transdecoder-old":
		return seqname.split("::")[1]
	elif predictor == "transdecoder-new":
		return seqname.split(".p")[0]
...
def parse_faa(assembler, predictor, fasta_aa):
	# scaflen can read info in seqnames from SPAdes assemblies
	# For Trinity assemblies, this scaffold lengths are inferred from nt assembly.
	# Other assemblers not yet supported
	faa_file = SeqIO.parse(fasta_aa, "fasta")
	faa_d = {}

	if assembler == "spades":
		if predictor in ("transdecoder-old", "transdecoder-new"):
			coordsregex = r":(\d+)-(\d+)\("
			for seq in faa_file:
		...
		if predictor in ("transdecoder-old", "transdecoder-new"):
			...
			for seq in faa_file:
				...
				coords = (float(re.search(coordsregex, header).group(1)), 
						  float(re.search(coordsregex, header).group(2)))
				coords = (min(coords), max(coords))
				scaflen = scaffolds_d.get(get_scaffold(predictor, header), {"length": 1})["length"]
				#print(coords, scaflen)
				genelen = coords[1] - coords[0] + 1
				faa_d[seq.name] = {"coords": coords, "gene_len": genelen, "scaf_len": scaflen}
		elif predictor == "prodigal":
...
def test():
	assembly_methods(">TRINITY_DN7724_c0_g1::TRINITY_DN7724_c0_g1_i1::g.1::m.1 type:3prime_partial len:132 gc:universal TRINITY_DN7724_c0_g1_i1:113-505(+)")
	assembly_methods(">c0_g2_i1.p1 type:3prime_partial len:319 gc:universal c0_g2_i1:175-1128(+)")
	assembly_methods("NODE_177_length_94900_cov_24.327530.p4")
